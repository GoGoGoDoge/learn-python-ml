...
from hac import HAC # , splitting_type
...
def cv_score(alpha=1., beta=0., k=5):
    ...
    for i in range(0,cv):
        ...
        # print(gm_train)
        #max_dis = max(max(dm_train))
        #len_dm = d - len(test_indices)
        #for dm_i in range(0,len_dm):
        #    dm_train[dm_i][dm_i] = max_dis+1

        #min_dis = min(min(dm_train))
        #print("MIN DIS IS: ",min_dis)
        #cut_dis = min_dis + cut*(max_dis-min_dis)
        #print("CUT DIS IS: ",cut_dis)
        hac_instance = HAC(dm_train, k, 0.5, 20)
...
domain=[{'name':'alpha', 'type':'continuous', 'domain':(0,1)},
        ...
        {'name':'k', 'type':'discrete', 'domain':(3,45)}]
